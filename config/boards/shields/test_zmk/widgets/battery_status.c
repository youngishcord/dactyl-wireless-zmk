...
struct battery_widget_object
    battery_widget_objects[ZMK_SPLIT_BLE_PERIPHERAL_COUNT];
...
int zmk_widget_peripheral_battery_status_init(
    struct zmk_widget_peripheral_battery_status *widget, lv_obj_t *parent) {
  ...
  for (int i = 0; i < ZMK_SPLIT_BLE_PERIPHERAL_COUNT; i++) {
    ...
    lv_canvas_set_buffer(battery_widget_objects[i].battery_image_canvas,
                         battery_widget_objects[i].battery_image_buffer, 5, 8,
                         LV_IMG_CF_TRUE_COLOR);

    lv_obj_align(battery_widget_objects[i].battery_image_canvas,
                 LV_ALIGN_TOP_RIGHT, 0, i * 10);
    lv_obj_align(battery_widget_objects[i].battery_label, LV_ALIGN_TOP_RIGHT,
                 -7, i * 10);
  }
